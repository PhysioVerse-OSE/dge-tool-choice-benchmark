...
data = pd.read_csv('GSE161731-Ready.csv')
data.set_index('Gene Symbol', inplace=True)

groups = ['HLTY', 'Bacterial', 'Influenza']
group_dfs = {}
for group in groups:
    matching_cols = [col for col in data.columns if col.startswith(group)]
    group_dfs[group] = data[matching_cols]
control_df = group_dfs['HLTY']
Bacterial_df = group_dfs['Bacterial']
Influenza_df = group_dfs['Influenza']

Control_Bacterial = pd.concat([control_df, Bacterial_df], axis=1)
Control_Influenza = pd.concat([control_df, Influenza_df], axis=1)
...
def read_csv_files_to_dict(directory):
    pattern = os.path.join(directory, 'EdgeR_HLTY*_vs_*.csv')
...
Data = dataframes['EdgeR_HLTY_vs_Bacterial']
Data.rename(columns={'GeneSymbol': 'Gene Symbol'}, inplace=True)
title = 'EdgeR_HLTY_vs_Bacterial'
...
Bonferroni_Upregulated.to_csv('EdgeR_Upregulated_Bacterial.csv', index=False)
Bonferroni_Downregulated.to_csv('EdgeR_Downregulated_Bacterial.csv', index=False)
Bonferroni_Significant.to_csv('EdgeR_Significant_Bacterial.csv', index=False)

Data = dataframes['EdgeR_HLTY_vs_Influenza']
Data.rename(columns={'GeneSymbol': 'Gene Symbol'}, inplace=True)
title = 'EdgeR_HLTY_vs_Influenza'
config_Upset = {'method': 'Bonferroni', 'alpha': 0.05, 'logFC_threshold': 1, 'up_or_down_or_both': 'both_up_and_down_regulated', 'highlight_genes': [], 'top_genes': 20, 'upreg_criteria': None, 'downreg_criteria': None, 'upregulated_color': '#FF0000', 'downregulated_color': '#0000FF', 'not_significant_color': '#808080', 'not_significant_between_logFC_threshold_color': '#A0A0A0', 'pie_chart_size': 0.2, 'pie_chart_position': (0.85, 0.85), 'ranking_method': 'MAS', 'Magnitude (M)': 0, 'Altitude (A)': 1, 'g100': (-1, 0.2), 'g010': (1, 0.24), 'g001': (-0.4, -0.5), 'g110': (-0.6, 0.24), 'g101': (-0.8, -0.3), 'g011': (0.7, -0.1), 'g111': (0.4, -0.6)}
AllGenes_DE_HLTY_vs_SARS_CoV_2, Significant_DE_HLTY_vs_SARS_CoV_2 = volcano_plot_no_visualization(Data, M=config_Upset['Magnitude (M)'], A=config_Upset['Altitude (A)'], title=title, method=config_Upset['method'], alpha=config_Upset['alpha'], logFC_threshold=config_Upset['logFC_threshold'], highlight_genes=config_Upset['highlight_genes'], top_genes=config_Upset['top_genes'], upreg_criteria=config_Upset['upreg_criteria'], downreg_criteria=config_Upset['downreg_criteria'], upregulated_color=config_Upset['upregulated_color'], downregulated_color=config_Upset['downregulated_color'], not_significant_color=config_Upset['not_significant_color'], not_significant_between_logFC_threshold_color=config_Upset['not_significant_between_logFC_threshold_color'], pie_chart_size=config_Upset['pie_chart_size'], pie_chart_position=config_Upset['pie_chart_position'], ranking_method=config_Upset['ranking_method'], up_or_down_or_both=config_Upset['up_or_down_or_both'], g100=config_Upset['g100'], g010=config_Upset['g010'], g001=config_Upset['g001'], g110=config_Upset['g110'], g101=config_Upset['g101'], g011=config_Upset['g011'], g111=config_Upset['g111'])
config = {'method': 'Bonferroni', 'alpha': 0.05, 'logFC_threshold': 1, 'up_or_down_or_both': 'both_up_and_down_regulated', 'highlight_genes': [], 'top_genes': 20, 'upreg_criteria': None, 'downreg_criteria': None, 'upregulated_color': '#FF0000', 'downregulated_color': '#0000FF', 'not_significant_color': '#808080', 'not_significant_between_logFC_threshold_color': '#A0A0A0', 'pie_chart_size': 0.2, 'pie_chart_position': (0.85, 0.85), 'ranking_method': 'MAS', 'Magnitude (M)': 0, 'Altitude (A)': 1, 'g100': (-1, 0.2), 'g010': (1, 0.24), 'g001': (-0.4, -0.5), 'g110': (-0.6, 0.24), 'g101': (-0.8, -0.3), 'g011': (0.7, -0.1), 'g111': (0.4, -0.6)}
logFC_threshold_values = range(1, 5)
...
Bonferroni_Downregulated = Significant_DE_HLTY_vs_SARS_CoV_2[Significant_DE_HLTY_vs_SARS_CoV_2['logFC'] < -1]
Bonferroni_Significant = Significant_DE_HLTY_vs_SARS_CoV_2.copy()
Bonferroni_Upregulated.to_csv('EdgeR_Upregulated_Influenza.csv', index=False)
Bonferroni_Downregulated.to_csv('EdgeR_Downregulated_Influenza.csv', index=False)
Bonferroni_Significant.to_csv('EdgeR_Significant_Influenza.csv', index=False)
...
import pandas as pd
csv_files = ['EdgeR_Significant_Bacterial.csv', 'EdgeR_Significant_Influenza.csv']
...
def read_csv_files_to_dict(directory):
    pattern = os.path.join(directory, 'DESeq2_HLTY*_vs_*.csv')
    csv_files = glob.glob(pattern)
    if not csv_files:
        print('No matching files found in the directory.')
        return {}
...
Data = dataframes['DESeq2_HLTY_vs_Bacterial_with_FDR_and_Bonferroni']
Data.rename(columns={'GeneSymbol': 'Gene Symbol'}, inplace=True)
title = 'DESeq2_HLTY_vs_Bacterial_with_FDR_and_Bonferroni'
config_Upset = {'method': 'Bonferroni', 'alpha': 0.05, 'logFC_threshold': 1, 'up_or_down_or_both': 'both_up_and_down_regulated', 'highlight_genes': [], 'top_genes': 20, 'upreg_criteria': None, 'downreg_criteria': None, 'upregulated_color': '#FF0000', 'downregulated_color': '#0000FF', 'not_significant_color': '#808080', 'not_significant_between_logFC_threshold_color': '#A0A0A0', 'pie_chart_size': 0.2, 'pie_chart_position': (0.85, 0.85), 'ranking_method': 'MAS', 'Magnitude (M)': 0, 'Altitude (A)': 1, 'g100': (-1, 0.2), 'g010': (1, 0.24), 'g001': (-0.4, -0.5), 'g110': (-0.6, 0.24), 'g101': (-0.8, -0.3), 'g011': (0.7, -0.1), 'g111': (0.4, -0.6)}
AllGenes_DE_HLTY_vs_SARS_CoV_2, Significant_DE_HLTY_vs_SARS_CoV_2 = volcano_plot_no_visualization(Data, M=config_Upset['Magnitude (M)'], A=config_Upset['Altitude (A)'], title=title, method=config_Upset['method'], alpha=config_Upset['alpha'], logFC_threshold=config_Upset['logFC_threshold'], highlight_genes=config_Upset['highlight_genes'], top_genes=config_Upset['top_genes'], upreg_criteria=config_Upset['upreg_criteria'], downreg_criteria=config_Upset['downreg_criteria'], upregulated_color=config_Upset['upregulated_color'], downregulated_color=config_Upset['downregulated_color'], not_significant_color=config_Upset['not_significant_color'], not_significant_between_logFC_threshold_color=config_Upset['not_significant_between_logFC_threshold_color'], pie_chart_size=config_Upset['pie_chart_size'], pie_chart_position=config_Upset['pie_chart_position'], ranking_method=config_Upset['ranking_method'], up_or_down_or_both=config_Upset['up_or_down_or_both'], g100=config_Upset['g100'], g010=config_Upset['g010'], g001=config_Upset['g001'], g110=config_Upset['g110'], g101=config_Upset['g101'], g011=config_Upset['g011'], g111=config_Upset['g111'])
config = {'method': 'Bonferroni', 'alpha': 0.05, 'logFC_threshold': 1, 'up_or_down_or_both': 'both_up_and_down_regulated', 'highlight_genes': [], 'top_genes': 20, 'upreg_criteria': None, 'downreg_criteria': None, 'upregulated_color': '#FF0000', 'downregulated_color': '#0000FF', 'not_significant_color': '#808080', 'not_significant_between_logFC_threshold_color': '#A0A0A0', 'pie_chart_size': 0.2, 'pie_chart_position': (0.85, 0.85), 'ranking_method': 'MAS', 'Magnitude (M)': 0, 'Altitude (A)': 1, 'g100': (-1, 0.2), 'g010': (1, 0.24), 'g001': (-0.4, -0.5), 'g110': (-0.6, 0.24), 'g101': (-0.8, -0.3), 'g011': (0.7, -0.1), 'g111': (0.4, -0.6)}
logFC_threshold_values = range(1, 5)
...
Bonferroni_Downregulated = Significant_DE_HLTY_vs_SARS_CoV_2[Significant_DE_HLTY_vs_SARS_CoV_2['logFC'] < -1]
Bonferroni_Significant = Significant_DE_HLTY_vs_SARS_CoV_2.copy()
Bonferroni_Upregulated.to_csv('DESeq2_Upregulated_Bacterial.csv', index=False)
Bonferroni_Downregulated.to_csv('DESeq2_Downregulated_Bacterial.csv', index=False)
Bonferroni_Significant.to_csv('DESeq2_Significant_Bacterial.csv', index=False)

Data = dataframes['DESeq2_HLTY_vs_Influenza_with_FDR_and_Bonferroni']
Data.rename(columns={'GeneSymbol': 'Gene Symbol'}, inplace=True)
title = 'DESeq2_HLTY_vs_Influenza_with_FDR_and_Bonferroni'
config_Upset = {'method': 'Bonferroni', 'alpha': 0.05, 'logFC_threshold': 1, 'up_or_down_or_both': 'both_up_and_down_regulated', 'highlight_genes': [], 'top_genes': 20, 'upreg_criteria': None, 'downreg_criteria': None, 'upregulated_color': '#FF0000', 'downregulated_color': '#0000FF', 'not_significant_color': '#808080', 'not_significant_between_logFC_threshold_color': '#A0A0A0', 'pie_chart_size': 0.2, 'pie_chart_position': (0.85, 0.85), 'ranking_method': 'MAS', 'Magnitude (M)': 0, 'Altitude (A)': 1, 'g100': (-1, 0.2), 'g010': (1, 0.24), 'g001': (-0.4, -0.5), 'g110': (-0.6, 0.24), 'g101': (-0.8, -0.3), 'g011': (0.7, -0.1), 'g111': (0.4, -0.6)}
AllGenes_DE_HLTY_vs_SARS_CoV_2, Significant_DE_HLTY_vs_SARS_CoV_2 = volcano_plot_no_visualization(Data, M=config_Upset['Magnitude (M)'], A=config_Upset['Altitude (A)'], title=title, method=config_Upset['method'], alpha=config_Upset['alpha'], logFC_threshold=config_Upset['logFC_threshold'], highlight_genes=config_Upset['highlight_genes'], top_genes=config_Upset['top_genes'], upreg_criteria=config_Upset['upreg_criteria'], downreg_criteria=config_Upset['downreg_criteria'], upregulated_color=config_Upset['upregulated_color'], downregulated_color=config_Upset['downregulated_color'], not_significant_color=config_Upset['not_significant_color'], not_significant_between_logFC_threshold_color=config_Upset['not_significant_between_logFC_threshold_color'], pie_chart_size=config_Upset['pie_chart_size'], pie_chart_position=config_Upset['pie_chart_position'], ranking_method=config_Upset['ranking_method'], up_or_down_or_both=config_Upset['up_or_down_or_both'], g100=config_Upset['g100'], g010=config_Upset['g010'], g001=config_Upset['g001'], g110=config_Upset['g110'], g101=config_Upset['g101'], g011=config_Upset['g011'], g111=config_Upset['g111'])
config = {'method': 'Bonferroni', 'alpha': 0.05, 'logFC_threshold': 1, 'up_or_down_or_both': 'both_up_and_down_regulated', 'highlight_genes': [], 'top_genes': 20, 'upreg_criteria': None, 'downreg_criteria': None, 'upregulated_color': '#FF0000', 'downregulated_color': '#0000FF', 'not_significant_color': '#808080', 'not_significant_between_logFC_threshold_color': '#A0A0A0', 'pie_chart_size': 0.2, 'pie_chart_position': (0.85, 0.85), 'ranking_method': 'MAS', 'Magnitude (M)': 0, 'Altitude (A)': 1, 'g100': (-1, 0.2), 'g010': (1, 0.24), 'g001': (-0.4, -0.5), 'g110': (-0.6, 0.24), 'g101': (-0.8, -0.3), 'g011': (0.7, -0.1), 'g111': (0.4, -0.6)}
logFC_threshold_values = range(1, 5)
...
Bonferroni_Downregulated = Significant_DE_HLTY_vs_SARS_CoV_2[Significant_DE_HLTY_vs_SARS_CoV_2['logFC'] < -1]
Bonferroni_Significant = Significant_DE_HLTY_vs_SARS_CoV_2.copy()
Bonferroni_Upregulated.to_csv('DESeq2_Upregulated_Influenza.csv', index=False)
Bonferroni_Downregulated.to_csv('DESeq2_Downregulated_Influenza.csv', index=False)
Bonferroni_Significant.to_csv('DESeq2_Significant_Influenza.csv', index=False)
...
import pandas as pd
csv_files = ['DESeq2_Significant_Bacterial.csv', 'DESeq2_Significant_Influenza.csv']
ranked_dataframes = {filename: pd.read_csv(filename) for filename in csv_files}
print(ranked_dataframes.keys())
...
def load_and_process_data():
    file_names = ['EdgeR_Significant_Influenza.csv']
...
def load_and_process_data():
    file_names = ['EdgeR_Significant_Bacterial.csv']
    current_dir = os.getcwd()
    for file_name in file_names:
        file_path = os.path.join(current_dir, file_name)
        if os.path.exists(file_path):
...
def load_and_process_data():
    file_names = ['DESeq2_Significant_Influenza.csv']
    current_dir = os.getcwd()
    for file_name in file_names:
        file_path = os.path.join(current_dir, file_name)
        if os.path.exists(file_path):
...
def load_and_process_data():
    file_names = ['DESeq2_Significant_Bacterial.csv']
    current_dir = os.getcwd()
    for file_name in file_names:
        file_path = os.path.join(current_dir, file_name)
        if os.path.exists(file_path):
...
edgeR_sig_df = pd.read_csv('EdgeR_Significant_Bacterial.csv').set_index('Gene Symbol')
deseq2_sig_df = pd.read_csv('DESeq2_Significant_Bacterial.csv').set_index('Gene Symbol')
...
ax.set_title('Bacterial', fontsize=18, weight='bold', pad=20)
ax.set_xticks(x)
...
def prepare_data(gene_list, df):
    ...
    group_names = ['Control' if 'HLTY' in s else 'Bacterial' for s in data_transposed.index]
...
color_map = {'Control': 'green', 'Bacterial': 'c'}
data_scaled_er, labels_er, group_names_er = prepare_data(edgeR_only_up + edgeR_only_down, Control_Bacterial)
data_scaled_dq, labels_dq, group_names_dq = prepare_data(deseq2_only_up + deseq2_only_down, Control_Bacterial)
...
plot_conf_matrix(ax3, cm_er, 'Confusion Matrix: edgeR-unique', cmap='Blues', class_labels=['Control', 'Bacterial'], y_true=labels_er, y_pred=preds_er)
ax4 = fig.add_subplot(gs[1, 1])
plot_conf_matrix(ax4, cm_dq, 'Confusion Matrix: DESeq2-unique', cmap='Reds', class_labels=['Control', 'Bacterial'], y_true=labels_dq, y_pred=preds_dq)
plt.tight_layout()
plt.show()
...
import seaborn as sns
from scipy.stats import pearsonr, spearmanr
edgeR_sig_df = pd.read_csv('EdgeR_Significant_Influenza.csv').set_index('Gene Symbol')
deseq2_sig_df = pd.read_csv('DESeq2_Significant_Influenza.csv').set_index('Gene Symbol')
edgeR_up = set(edgeR_sig_df[edgeR_sig_df['Category'] == 'Upregulated'].index)
edgeR_down = set(edgeR_sig_df[edgeR_sig_df['Category'] == 'Downregulated'].index)
deseq2_up = set(deseq2_sig_df[deseq2_sig_df['Category'] == 'Upregulated'].index)
deseq2_down = set(deseq2_sig_df[deseq2_sig_df['Category'] == 'Downregulated'].index)
...
bar3 = ax.bar(x + width, deseq2_only_counts, width, label='Only in DESeq2', color=colors['Only in DESeq2'])
ax.set_ylabel('Number of Significant Genes with \n Bonferroni adjusted p-value <0.05, \n |LogFC| > 1', fontsize=18, weight='bold')
ax.set_title('Influenza', fontsize=18, weight='bold', pad=20)
ax.set_xticks(x)
ax.set_xticklabels(labels, fontsize=18, weight='bold')
ax.tick_params(axis='y', labelsize=18)
ax.spines['top'].set_visible(False)
ax.spines['right'].set_visible(False)
ax.legend(frameon=True, fontsize=18, loc='upper left', bbox_to_anchor=(0.4, 0.5))
for bars in [bar1, bar2, bar3]:
    for bar in bars:
        height = bar.get_height()
        ax.annotate(f'{int(height)}', xy=(bar.get_x() + bar.get_width() / 2, height), xytext=(0, 5), textcoords='offset points', ha='center', va='bottom', fontsize=18, weight='bold')
textstr = f'\nlogFC Correlation:\n  Pearson r = {pearson_logfc:.3f};   Spearman ρ = {spearman_logfc:.3f}\nBonferroni Correlation:\n  Pearson r = {pearson_bonf:.3f};   Spearman ρ = {spearman_bonf:.3f}\nJaccard Index:\n  Upregulated = {jaccard_up:.3f};  Downregulated = {jaccard_down:.3f}\nContradictory Regulation = {contradictory}'
props = dict(boxstyle='round,pad=0.8', facecolor='white', edgecolor='black')
ax.text(0.45, 1, textstr, transform=ax.transAxes, fontsize=18, verticalalignment='top', bbox=props)
plt.tight_layout()
plt.show()
common_up = edgeR_up & deseq2_up
common_down = edgeR_down & deseq2_down
...
def prepare_data(gene_list, df):
    ...
    data = data.loc[filtered_genes]
    data_transposed = np.log2(data.transpose() + 1)
    group_names = ['Control' if 'HLTY' in s else 'Influenza' for s in data_transposed.index]
    labels = [0 if g == 'Control' else 1 for g in group_names]
    return (data_transposed, labels, group_names)
...
def plot_conf_matrix(ax, cm, title, cmap, class_labels, y_true, y_pred):
    ...
    ax.set_ylabel('True Label', fontsize=16, fontweight='bold', labelpad=12)
    ax.tick_params(axis='both', labelsize=14, width=2)
color_map = {'Control': 'green', 'Influenza': 'blue'}
data_scaled_er, labels_er, group_names_er = prepare_data(edgeR_only_up + edgeR_only_down, Control_Influenza)
data_scaled_dq, labels_dq, group_names_dq = prepare_data(deseq2_only_up + deseq2_only_down, Control_Influenza)
fig = plt.figure(figsize=(12, 12), dpi=120)
gs = gridspec.GridSpec(2, 2, height_ratios=[1.2, 1])
ax1 = fig.add_subplot(gs[0, 0])
components_er = plot_pca(ax1, data_scaled_er, group_names_er, 'PCA: edgeR-unique Genes', color_map)
